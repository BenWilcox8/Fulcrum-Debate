...
| `flow-sheet` | `nodes` | `Y.Map<Y.Map>` | Flow nodes keyed by id; each map records `columnId` (membership), `kind`, and `order` (vertical). See [Flow sheet node-container contract](#flow-sheet-node-container-contract). |
...
`src/flow/canvas/` holds the render surface (`FlowCanvas`), the column-management write strip (`ColumnControls`), and the composited editable panel (`FlowSheetPanel`), plus the live column->node seam and the [node-container hosting seam](#flow-sheet-node-container-contract) (`node-host.ts`, `useFlowNodes.ts`).
It ships **no flow-node content** (contentions, subpoints, argument rows are later PRDs) - just the column rendering and the hosting contract those PRDs build on.

- **`FlowCanvas` is the render-only canvas** (`FlowCanvas.tsx`, import from `src/flow/canvas`): give it a flow-sheet `DocumentHandle` (or `null` while opening) and it renders a `<ReactFlow>` surface. Feature code that needs both rendering and column management should use `FlowSheetPanel` instead. Columns are populated live via the model's `observeColumns`, so nothing here awaits a network resource - it paints synchronously and fills in columns once IndexedDB has loaded into the doc (the local-first boot rule). It measures its own viewport height with a `ResizeObserver` purely so columns render full-height; measurement never gates when columns appear.
...
- **Column node = a container seam:** `SpeechColumnNode.tsx` renders a labelled header + an empty `data-column-body` region - the deliberate visual seam for hosted flow nodes. The membership/hosting mechanism (XYFlow child nodes parented to the column) is the [Flow sheet node-container contract](#flow-sheet-node-container-contract); flow nodes are positioned as XYFlow children over the column, not literally mounted into this DOM region. Side colouring is **design-token classes only** (`bg-aff-soft`/`border-aff-strong`, `bg-neg-soft`/`border-neg-strong`), never raw hex in the component, keyed off `data.side`.
...
## Flow sheet column management (the write path)

`FlowCanvas` is render-only; the *write* path that adds, labels, reorders, and removes speech columns lives beside it in `src/flow/canvas/` and drives the [flow-sheet column model](#flow-sheet-column-model) helpers directly.
It ships no flow-node content and does not touch the column-node/container seam - it only mutates the column list.

- **`ColumnControls` is the write strip** (`ColumnControls.tsx`, import from `src/flow/canvas`): give it a flow-sheet `DocumentHandle` (or `null` while opening) and it renders a minimal, shell-idiomatic control panel - an add form (aff/neg side toggle + label input, submit disabled until a handle exists and the trimmed label is non-empty) and, per column, an inline label editor plus reorder and remove actions. Every gesture calls a `src/flow` helper (`addColumn`/`relabelColumn`/`moveColumn`/`removeColumn`), so all writes persist to IndexedDB and flow through the shared `observeColumns` seam. The strip holds no column state of its own; its list comes from `useColumns`.
- **Reorder is button-driven, not drag:** each row has ← / → buttons calling `moveColumn(handle, id, index ∓ 1)` (disabled at the ends). Full column drag-and-drop is deliberately out of scope; step buttons fit the shell's plain style and exercise the same `moveColumn` path a future drag would.
- **Inline relabel commits on Enter or blur, cancels on Escape:** `ColumnRow` keeps a per-column `draft` (a `useState` reset via effect when `column.label` changes, so an external edit or a reorder that reuses the row for a different column re-syncs it). Commit calls `relabelColumn` only when the trimmed draft is non-empty and actually changed. **Enter commits `draft` directly** (then blurs) rather than relying on the blur handler - a programmatic `.blur()` on a non-focused input fires no blur event under jsdom, so an Enter-triggers-blur-triggers-commit chain would silently drop the edit in tests. Blur commits independently for click-away.
- **`useColumns(handle)`** (`useColumns.ts`) is the read side the controls consume: the `SpeechColumn`-model mirror of `useColumnNodes`, driving the same `observeColumns` seam so a control strip and the canvas always agree on the column list. A nullish handle yields `[]`.
- **`FlowSheetPanel`** (`FlowSheetPanel.tsx`) is the editable flow sheet: it stacks `ColumnControls` above `FlowCanvas` on one handle, adding no state, so the render-only canvas and the write strip stay in sync through `observeColumns`. This composition keeps `FlowCanvas`/`SpeechColumnNode` (the parallel node-container work) untouched.
- **Tests:** `column-controls.test.tsx` drives `ColumnControls` over a *real* flow-sheet handle (`fake-indexeddb/auto` + fresh `IDBFactory()` per test) with `fireEvent` (no `@testing-library/user-event` dependency), asserting each gesture lands in `listColumns` and that add/relabel/reorder survive a reload through a genuinely fresh handle. Assertions stay behavioral (model state / rendered rows), never pixels.

## Flow sheet node-container contract

The **node-container contract** is the stable seam every later flow-node PRD (contentions, subpoints, argument rows) builds on: how a flow node records which speech column it belongs to and where it sits, plus how a registered node kind is hosted inside its column on the canvas.
It is two halves - the flow-doc model (`src/flow/nodes.ts`) and the canvas hosting seam (`src/flow/canvas/node-host.ts` + `useFlowNodes.ts`) - and ships **no real node kinds, no drag/collapse/editing UI**, only the contract and a test stub.
The three constraints that keep it stable: membership lives in the flow doc (never transient XYFlow state), the public `FlowNode` snapshot stays content-free, and a node kind is added purely by registration (no canvas edits).

- **Model - membership + vertical order, content-free:** `src/flow/nodes.ts` owns the `nodes` fragment (a top-level `Y.Map<Y.Map>` on a `flow-sheet` doc, reserved in the fragment table). Each node is a nested `Y.Map` with `id`, `columnId`, `kind`, `order`. The public `FlowNode = { id, columnId, kind }` snapshot deliberately omits `order` - it is an internal sequencing detail, and callers get a node's vertical position from its index in `listColumnNodes`. **Membership** is the `columnId` foreign key (a `SpeechColumn.id`); a node whose column is gone is an *orphan* and simply is not rendered (no cascade here - that lifecycle is the consuming feature's call). **Vertical order** is the ascending `order` field, tie-broken by id for a stable total order.
- **Keyed `Y.Map`, not a `Y.Array` (the key design choice):** columns use a `Y.Array` and *rebuild* a map on reorder, which is fine for primitive fields but would destroy the rich content a node kind attaches (Yjs cannot re-position an integrated shared type). Keying `nodes` by id and expressing order in a plain `order` field means `moveNode` only rewrites primitive `order` values - the node's identity and attached content stay put. `nodes` is bound to `Y.Map` for the life of the doc; never re-type or rename it.
- **Helpers (import from `src/flow`):** `listNodes(handle)` (every node, stable total order by column then vertical then id), `listColumnNodes(handle, columnId)` (one column's nodes in vertical order - the index *is* the vertical position), `getNode`, `addNode(handle, {columnId, kind})` (mints id, appends to the column bottom), `moveNode(handle, id, toIndex)` (reorder within its column; clamps `toIndex`, throws on unknown id, no-op if unchanged; renumbers primitives only), `removeNode` (no-op on unknown id), `observeNodes(handle, listener)` (fires immediately then on every add/move/membership/remove; `observeDeep`, so a reorder inside a nested map fires; other fragments do not). Each mutates in one transaction, persists to IndexedDB, and flows through the doc `update` stream (a `track`ing registry bumps last-edited automatically).
- **Hosting seam - XYFlow parent/child, model-authoritative:** a registered kind renders as an XYFlow **child node** parented to its column node (`parentId` = the column id, `extent: "parent"` so it is clipped to the column's bounds), positioned by the flow-doc order - never by transient XYFlow state. The column id is the column node's XYFlow id, so `columnId` flows straight to `parentId`. `column-nodes.ts` and `node-host.ts` keep the pure model->node translation separate from the `<ReactFlow>` wiring (jsdom can't measure); child y-offset is a pure function of vertical rank (`flowNodeY`), needing no measurement because child coordinates are column-relative.
- **Registration API (extensible without touching the canvas):** a PRD adds a node kind by passing a `FlowNodeRegistry` (a list of `FlowNodeTypeDefinition = { kind, component, height? }`) to `FlowCanvas`'s `flowNodeTypes` prop. The canvas builds its XYFlow `nodeTypes` (`registryToNodeTypes`) and its hosted child nodes (`flowNodesToNodes` via `useFlowNodes`, which observes both nodes and columns) straight from that registry - it never enumerates kinds itself. A node whose `kind` has no registered definition is **skipped** (no renderer), so an unknown/orphaned/legacy node degrades gracefully. The custom node component receives `FlowNodeData = { flowNodeId, columnId, kind }` (a thin identity snapshot); a kind reads its own *content* from the flow doc by `flowNodeId`, not from `data`. Parent nodes precede children in the combined node array (an XYFlow requirement).
- **Tests:** the model (`nodes.test.ts`) and the pure host mapping (`node-host.test.ts`) follow the established pattern (`fake-indexeddb/auto` + fresh `IDBFactory()`, behavioral assertions on helpers, persisted state, and node objects). `node-host.integration.test.tsx` drives `useFlowNodes` and a rendered `FlowCanvas` over a *real* flow-sheet handle with a **test-only stub node kind** (defined inline, the minimal `{ kind, component }` implementation) - proving a registered kind is hosted inside its column (`parentId` == `columnId`), reacts live to add/reorder/remove per column, is skipped when unregistered, and renders in the DOM alongside its column. XYFlow renders custom child nodes under jsdom, so the rendered-canvas assertions hold; membership/clipping is asserted at the node-object level (`parentId`/`extent`), never by pixels.

## Round lifecycle (rounds <-> flow-sheet documents)

A **round is a flow-sheet document** - there is no separate round store or metadata.
`src/rounds/` is the thin seam that maps the round concept onto the existing document layer, and the shell routes onto it; this is integration, not new infrastructure.

- **A round's id *is* its flow-sheet document's id.** Creating a round mints a `flow-sheet` document through the document service (`create({ kind: "flow-sheet", title })`); the returned handle's id is the round id (the suffix of its own `fulcrum:doc:<id>` content database). Opening a round is `service.open(id)` / `useDocument(id)`; the round's columns are the [flow-sheet column model](#flow-sheet-column-model) on that document. **Round isolation is free**: each round is a separate document in its own IndexedDB database, so one round's flow-sheet edits can never surface in another.
- **`src/rounds/rounds.ts` is the whole seam:** `ROUND_KIND` (`"flow-sheet"`), `defaultRoundTitle(existingCount)`, and `useRounds()` - which layers directly on `useDocuments`, filtering the recency-ordered listing to `ROUND_KIND` and exposing `{ rounds, loading, createRound(title?), removeRound(id) }`. `createRound` returns the new round id so a caller can navigate straight to it; it defaults the title to `Round N` when none is given. No new registry, no new persistence - rounds *are* the flow-sheet documents.
- **Routing (`src/AppRoutes.tsx`):** `rounds` renders `RoundsScreen` (the index: a "New round" button that creates a round and `navigate`s to `/rounds/:id`, plus links to existing rounds), and the nested `rounds/:roundId` renders `RoundScreen` (opens the round via `useDocument(roundId)` and hands the handle to `FlowSheetPanel`). The `/rounds` NavLink is `end: false`, so it stays active on the nested route. `RoundScreen` paints synchronously and fills columns once the handle's local load resolves - nothing here awaits the network, so the local-first boot rule holds. If the id is not found in the registry after the local load completes, `RoundScreen` renders a "Round not found" message with a link back to `/rounds` rather than crashing or showing an empty canvas.
- **Providers stay where they are:** `DocumentsProvider` remains mounted in `main.tsx` (outside `App`) precisely so `App.offline-boot.test.tsx` renders `App` without constructing a service. Because the rounds screens now consume the document service, the routed-subtree tests in `App.test.tsx` (which mount `AppRoutes` bare) wrap it in a real `DocumentsProvider` + `fake-indexeddb/auto`; the top-level `render(<App/>)` test stays at Dashboard and needs no provider.
- **Tests:** `src/rounds/rounds.test.tsx` drives the round lifecycle end to end through the real service (`fake-indexeddb/auto` + fresh `IDBFactory()` per test): starting a round opens a fresh empty canvas backed by a new document; a round's columns are restored by reopening it through a *completely fresh provider* over the same backend (simulated restart); two rounds are proven isolated (each shows only its own column). Assertions are behavioral (rendered rows / column labels / listing links), never pixels.
