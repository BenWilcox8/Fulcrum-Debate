...
The pure model + preferences wiring for the product's house evidence formatting standards (slice 1 of the Evidence Formatting Standards & Customization feature). Model only - **no rendering, no Tiptap, no React**; sibling slices consume it (live card rendering, the unformatted-shrink rule, the Settings panel). Re-exported from `src/formatting`.
...
- **Tests:** `profile.test.ts` pins the target keys, the per-entry shape, the exact standard defaults, and scale-consistency (unformatted === `FONT_SIZE_SCALE[0]`, on-scale defaults are scale members). `preferences.test.ts` (`fake-indexeddb`) covers registration under the well-known id, defaults == standards, per-field labels, idempotent re-registration preserving set values, per-entry editing isolation, reset-restores-standards, and a real `openPreferenceStore` round-trip (set survives restart, unset resolves to standard, reset forgets).
