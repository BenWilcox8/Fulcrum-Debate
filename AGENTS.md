...
- **`CARD_TOOL_DEFINITIONS` (`src/tools/cardTools.ts`)** is the single app-level tool list (toolbar order), re-exported from `src/tools`. Both surfaces enumerate it: the editor toolbar registers each definition on a `createCardToolRegistry` (via `useCardTools`), and the Settings screen surfaces each one's settings (via `toolSettingsContributions`). Adding a tool here wires it into both at once. It currently contains `highlightCardTool`; the remaining tools (Extract, Shrink, Condense, Auto Speech, Send to Block File) are one-line additions as they land.
