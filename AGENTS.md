...
`src/flow/canvas/` holds the render surface (`FlowCanvas`), the column-management write strip (`ColumnControls`), and the composited editable panel (`FlowSheetPanel`), plus the live column->node seam.
It ships **no flow-node content or node-container API** (a later PRD establishes that contract on top of the column-node seam here).
