...
The model + preferences wiring for the product's house evidence formatting standards, plus this feature's Settings panel. The `profile.ts`/`preferences.ts` core is pure (**no rendering, no Tiptap, no React**); the Settings panel (`FormattingSettingsPanel.tsx`) is the one React surface. Sibling slices consume the profile (live card rendering, the unformatted-shrink rule). Re-exported from `src/formatting`.
