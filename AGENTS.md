...
- **`ResumeRecentZone`** (most prominent - an elevated bordered/shadowed card) is a **clean placeholder slot**: a sibling issue fills the recent-items list. Keep it as a seam - swap the placeholder body, don't rebuild the layout.
...
- **Tests:** `DashboardScreen.test.tsx` covers default-landing + no-connecting-gate, all three zones as labelled regions (`<section aria-labelledby>` → `role="region"`), Resume-first reading order, the placeholder recent slot, live/enabled create actions wired end-to-end (New round opens a round flow sheet; New card / Open block file open the block file), and Library navigation into both screens. `StartSomethingNewZone.test.tsx` unit-asserts the navigation targets; `NewRoundScreen.test.tsx` proves the create primitive runs and redirects.
