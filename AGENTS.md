...
- **`useCardTools()`** (`useCardTools.ts`) builds a `createCardToolRegistry` over the shared `PreferenceStore`, registers the shipped tools (`SHIPPED_CARD_TOOLS`, memoised per store), and returns `ToolbarTool[]`. **Provider-tolerant** like `useFormattingProfile`: with no `PreferenceStoreProvider` it falls back to a private store so the toolbar still paints. `register` is idempotent, so re-renders never re-register or throw. `SHIPPED_CARD_TOOLS` spreads `CARD_TOOL_DEFINITIONS` (the real tools with settings) then appends the settings-less toolbar-only tools (`condenseTool`, `extractHighlightTool`) then the `demoCardTool` - so adding a real tool with settings to `CARD_TOOL_DEFINITIONS` is the only wiring needed. Tools that need a custom control (Send) get `renderControl` attached here after registration; plain tools pass through unchanged.
...
- **`CARD_TOOL_DEFINITIONS` (`src/tools/cardTools.ts`)** is the single app-level tool list (toolbar order), re-exported from `src/tools`. Both surfaces enumerate it: the editor toolbar registers each definition on a `createCardToolRegistry` (via `useCardTools`), and the Settings screen surfaces each one's settings (via `toolSettingsContributions`). Adding a tool here wires it into both at once. It currently contains `shrinkCardTool`, `highlightCardTool`, and `sendToBlockFileTool` (the tools with settings); the settings-less `condenseTool`/`extractHighlightTool` are toolbar-only (in `SHIPPED_CARD_TOOLS`, not here), and Auto Speech is a future one-line addition.
